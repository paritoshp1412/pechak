...
- ~~**Vite's default `base:"/"` bakes root-absolute asset URLs into the build**...~~ **Fixed (live-site-breaking bug found and fixed this session):** the production build previously emitted `<script type="module" src="/assets/index-....js">` (root-absolute) into `dist/index.html`. Deployed under the GitHub Pages project subpath `https://paritoshp1412.github.io/pechak/`, the browser requested `https://paritoshp1412.github.io/assets/...`, got a 404, and the app's module script never loaded at all — meaning nothing in the page ever ran (no wizard, no working buttons, exactly as observed on the live site). Fixed by adding `vite.config.mjs` with `base: "./"` so all built asset URLs are relative and work at any subpath. Verified by serving a fresh `dist/` build under a simulated `/pechak/` subpath locally and confirming both the page and its JS asset resolve with 200 (they previously would have 404'd). Also changed the one hardcoded absolute path left in source `index.html` (`<script src="/src/xlsx-loader.js">` → `./src/xlsx-loader.js`) for consistency, though the build output path is governed by `base`, not this attribute. **Anyone who deployed a `dist/` build made before this fix needs to rebuild and redeploy** — this repo's `dist/` is gitignored and deployed manually/externally, so this fix only takes effect on the next build+deploy.
